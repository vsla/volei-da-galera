...
export type Pelada = {
  ...
  memberCount: number;
  /**
   * Grupo arquivado (0026): some da lista, ninguém entra mais, e nada
   * foi apagado. É o que o app oferece no lugar de "excluir pelada" —
   * três meses de sexta guardam nota e histórico de gente que nem usa
   * mais o app, e apagar isso pra limpar uma tela é troca ruim.
   */
  archived: boolean;
};
 ...
 *
 * Arquivada não entra: o app deixa o dono arquivar (0026) e a promessa é
 * que o grupo some da frente de todo mundo. Se a web continuasse
 * listando, arquivar não teria efeito nenhum aqui.
 */
export async function fetchPeladas(meId: string | null): Promise<Pelada[]> {
  const [{ data: peladas }, { data: members }] = await Promise.all([
    supabase.from("peladas").select("*").is("archived_at", null).order("name"),
    supabase
      .from("pelada_members")
      .select("pelada_id, player_id, role, status"),
  ]);
  ...
  return ((peladas ?? []) as Row[]).map((p) =>
    toPelada(
      p,
      mine.get(p.id as string) ?? null,
      counts.get(p.id as string) ?? 0,
    ),
  );
}

/**
 * Só os grupos em que EU estou, e só os vivos.
 *
 * A `fetchPeladas` devolve a tabela inteira — o que era inofensivo
 * quando o banco tinha uma pelada, a Prainha. Num app self-service ela
 * vaza o grupo de todo mundo pra todo mundo, e a home passou a filtrar
 * na mão. Aqui o filtro é do banco, e `meId = null` (conta nova, ainda
 * sem jogador) devolve lista vazia em vez de todos.
 */
export async function fetchMyPeladas(
  meId: string | null,
  /** `true` devolve os arquivados em vez dos vivos — a tela de desarquivar. */
  arquivados = false,
): Promise<Pelada[]> {
  if (!meId) return [];
  ...
  const papeis = new Map<string, Role>(
    ((minhas ?? []) as Row[]).map((m) => [
      m.pelada_id as string,
      (m.role as Role) ?? "player",
    ]),
  );
  if (papeis.size === 0) return [];

  const ids = [...papeis.keys()];
  const alvo = supabase.from("peladas").select("*").in("id", ids);
  const [{ data: peladas }, { data: membros }] = await Promise.all([
    (arquivados ? alvo.not("archived_at", "is", null) : alvo.is("archived_at", null)).order(
      "name",
    ),
    supabase
      .from("pelada_members")
  ...
  return ((peladas ?? []) as Row[]).map((p) =>
    toPelada(p, papeis.get(p.id as string) ?? null, counts.get(p.id as string) ?? 0),
  );
}
...
export async function createPelada(input: {
  ...
  /** Quem você é neste aparelho, se já tiver jogado antes. */
  ...
  return row
    ? {
        id: row.id as string,
        slug: row.slug as string,
        // `player` na 0018 corrigida, `player_id` na primeira versão dela:
        // aceita as duas pra migration e deploy não terem ordem obrigatória
        playerId: (row.player ?? row.player_id) as string,
      }
    : null;
}

/**
 * O jogador guardado neste aparelho ainda existe no banco?
 *
 * Depois de um reset da lista, todo celular fica com um id que não
 * aponta pra ninguém. Sem esta checagem a home acha que já sabe quem
 * você é, não pergunta seu nome, e criar pelada falha no banco.
 */
 ...
 *
 * `joinPeladaByCode` escreve: cria um jogador e uma filiação. Isso
 * invertia a ordem das perguntas — a pessoa virava "mais um Miguel"
 * antes de alguém perguntar se ela não era o Miguel que já estava na
 * lista. Com a prévia, a tela mostra no que ela está prestes a entrar,
 * pergunta quem ela é, e só então escreve.
 *
 * `null` = código não existe.
 */
export async function peekPelada(code: string): Promise<PeladaPreview | null> {
  const { data, error } = await supabase.rpc("peek_pelada", {
...
export type Claimable = { playerId: string; name: string; rating: number };

/**
 * Os nomes DAQUELE grupo que ainda não têm dono.
 *
 * A pergunta "qual desses é você?" é do grupo, não da conta — e essa é
 * a correção inteira do fluxo antigo, que perguntava no login e lia a
 * `peladas` inteira pra montar a lista. Num app self-service isso é
 * oferecer a lista de estranhos pra quem acabou de se cadastrar.
 */
export async function claimableMembers(peladaId: string): Promise<Claimable[]> {
  ...
  return ((data as Row[] | null) ?? []).map((r) => ({
    playerId: (r.player_id ?? r.playerid) as string,
...
/**
 * ─────────────────────────────────────────────────────────────
 * GESTÃO DO GRUPO (0026) — o que só quem organiza pode fazer
 * ─────────────────────────────────────────────────────────────
 *
 * Tudo aqui passa por função do banco com checagem de papel, ao
 * contrário do resto do arquivo, que escreve direto na tabela. Não é
 * inconsistência: a `0022` trocou RLS por confiança entre amigos, e isso
 * continua valendo pra marcar ponto e gerar time. Mas estas apagam — e
 * apagar é o único lugar onde "qualquer um pode" não serve.
 *
 * Consequência prática: dependem de SESSÃO. São telas do app, e a web
 * anônima não as oferece.
 */

/**
 * Tirar alguém do grupo. Devolve o que de fato aconteceu.
 *
 *   `removed`  — a pessoa jogou. O nome dela está nas partidas de quem
 *                ficou, e apagar levaria o histórico deles junto.
 *   `deleted`  — nome digitado errado, ou alguém que entrou no grupo
 *                trocado: sem conta, sem partida, sem voto e em nenhum
 *                outro grupo. Não há o que preservar.
 *
 * Quem decide qual dos dois é o banco, não a tela — a tela só conta a
 * verdade depois, porque "saiu da lista" e "apagado de vez" não são a
 * mesma promessa.
 */
export async function removePeladaMember(
  peladaId: string,
  playerId: string,
): Promise<"removed" | "deleted"> {
  const { data, error } = await supabase.rpc("remove_pelada_member", {
    p_pelada: peladaId,
    p_player: playerId,
  });
  if (error) throw new Error(error.message);
  return data === "deleted" ? "deleted" : "removed";
}

/** Sair do grupo por conta própria. O dono precisa passar a bola antes. */
export async function leavePelada(peladaId: string) {
  const { error } = await supabase.rpc("leave_pelada", { p_pelada: peladaId });
  if (error) throw new Error(error.message);
}

/** Arquivar (ou desarquivar). Só o dono. Nada é apagado. */
export async function archivePelada(peladaId: string, on = true) {
  const { error } = await supabase.rpc("archive_pelada", {
    p_pelada: peladaId,
    p_on: on,
  });
  if (error) throw new Error(error.message);
...
/** Passar o grupo pra outra pessoa. Quem passa continua admin. */
export async function transferPeladaOwner(peladaId: string, playerId: string) {
  const { error } = await supabase.rpc("transfer_pelada_owner", {
    p_pelada: peladaId,
    p_player: playerId,
  });
  if (error) throw new Error(error.message);
...
export async function regenerateJoinCode(peladaId: string): Promise<string> {
 ...
 *
 * O `slug` fica como está, de propósito: ele já está no link que caiu no
 * WhatsApp, e link que morre porque alguém arrumou um acento é pior que
 * um endereço desatualizado.
 */
export async function renamePelada(
  peladaId: string,
  name: string,
  weekday: number | null,
) {
  const { error } = await supabase.rpc("rename_pelada", {
    p_pelada: peladaId,
    p_name: name,
    p_weekday: weekday,
  });
...
/**
 * Cadastra um nome na pelada. Devolve o `player_id`.
 *
 * Passa pela `add_member` (0023), não por dois inserts do cliente: a
 * escrita em `players` e a em `pelada_members` dependem uma da outra, e
 * jogador criado sem virar membro não aparece em tela nenhuma — some
 * sem erro. Idempotente: o mesmo nome duas vezes devolve o mesmo id.
 */
...
/**
 * A lista da semana inteira, um nome por vez.
 *
 * Sequencial de propósito, e não é economia de código: `add_member`
 * decide "esse nome já está aqui?" lendo a tabela, então dois nomes
 * parecidos saindo juntos podem os dois achar que são novos e criar
 * dois jogadores. Em série, o segundo enxerga o primeiro.
 *
 * Se cair no meio, o que entrou fica e o resto não — e tentar de novo é
 * seguro, porque quem já entrou é reconhecido em vez de duplicado. Por
 * isso o erro sobe com quantos já tinham passado.
 */
export async function addMembers(
  peladaId: string,
  names: { name: string; isGuest: boolean }[],
): Promise<string[]> {
  const ids: string[] = [];
  for (const n of names) {
    try {
      ids.push(await addMember(peladaId, n.name, n.isGuest));
    } catch (e) {
      const msg = e instanceof Error ? e.message : "erro";
      throw new Error(
        `Parou em "${n.name}" (${ids.length} de ${names.length} já entraram): ${msg}`,
      );
    }
  }
  return ids;
...
/**
 * Quem não está na lista colada sai da tela — `status = 'removed'`.
 *
 * Devolve QUANTOS saíram, não quem: a tela já sabe os nomes (ela mostrou
 * a prévia antes de gravar), e o número é o que serve pra conferir que
 * o banco entendeu a mesma coisa que a prévia mostrou.
 *
 * Nada é apagado: nota, histórico e partidas ficam. `owner` e `admin`
 * são imunes, na própria função do banco (0023).
 */
export async function syncMembers(
  peladaId: string,
  names: string[],
): Promise<number> {
  const { data, error } = await supabase.rpc("sync_members", {
    p_pelada: peladaId,
    p_names: names,
  });
  if (error) throw new Error(error.message);
...
/**
 * ─────────────────────────────────────────────────────────────
 * IDENTIDADE: o nome da lista vira uma conta
 * ─────────────────────────────────────────────────────────────
 *
 * A ordem do mundo real é essa, e ela manda no desenho: a lista chega do
 * WhatsApp na sexta de manhã, então `players` nasce SEM DONO. A conta
 * aparece depois — se aparecer. Quem cria conta não ganha um jogador
 * novo: reivindica um que já está lá.
 *
 * Todo mundo PODE logar, ninguém PRECISA. Convidado nunca precisa.
 */

/**
 * Trocar o nome de um jogador.
 *
 * Vale na pelada inteira e retroativamente, inclusive nas sextas que já
 * passaram — e isso é o certo, não um efeito colateral: é a MESMA
 * pessoa, e uma partida de 04/09 jogada por quem hoje se chama outra
 * coisa continua tendo sido jogada por ela.
 *
 * Quem pode: a própria pessoa (pela tela da conta) e quem organiza
 * (pelo painel). O banco não distingue — §9: a `0022` trocou RLS por
 * confiança entre amigos, e isto não muda essa decisão.
 */
export async function renamePlayer(playerId: string, name: string) {
  const limpo = name.trim().replace(/\s+/g, " ");
  if (limpo.length < 2) throw new Error("O nome precisa de pelo menos 2 letras.");
...
/**
 * Reivindicar um nome da lista — o auto-serviço.
 *
 * `claim_player` (0013) só deixa se `user_id is null`: nome que já tem
 * dono não é reivindicável, e é essa a trava toda. Devolve `false` quando
 * recusou, e a tela precisa dizer por quê em vez de sumir com o erro.
 *
 * ⚠️ Uma conta, um jogador. Reivindicar um segundo nome não divide a
 * pessoa em duas — a função devolve `false` e o jogador continua sendo
 * o primeiro.
 */
...
/**
 * O convite do organizador — o caminho autorizado.
 *
 * O claim livre acima tem o furo do §9b do `reasonable.md` ("alguém votou
 * a noite inteira como outra pessoa"), e agora, com conta, o erro seria
 * PERMANENTE em vez de um `localStorage` que se limpa. Quando quem monta
 * a lista gera o link, o claim passa a ser autorizado por ela.
 *
 * Devolve `null` quando o token não existe ou já foi usado — que é o
 * caso comum de "mandei o link no grupo e duas pessoas clicaram".
 */
export async function claimInvite(
  token: string,
): Promise<{ peladaId: string; slug: string; playerId: string } | null> {
  ...
  return row
    ? {
        peladaId: row.id as string,
        slug: row.slug as string,
        // `player` desde a 0025; `player_id` na versão anterior dela.
        // Aceita os dois pra migration e deploy não terem ordem.
        playerId: (row.player ?? row.player_id) as string,
      }
    : null;
}
...
/**
 * Gera (ou renova) o convite de uma pessoa da lista.
 *
 * `admin_add_roster_member` exige `is_pelada_admin()`, que por sua vez
 * exige que QUEM CHAMA esteja logado e com o próprio jogador já
 * reivindicado. É de propósito: é o que separa este caminho do
 * auto-serviço. Sem isso o banco recusa — inclusive pro dono da pelada.
 *
 * Chamar de novo pra mesma pessoa troca o token e desfaz o anterior.
 */
export async function inviteMember(
  ...
  const row = (data as Row[] | null)?.[0];
  if (!row) throw new Error("O banco não devolveu o convite.");
  return {
    playerId: (row.player ?? row.player_id) as string,
...
export async function addGuest(
  ...
  // uma chamada só (`join_as_guest`): cria o jogador e o torna membro no
  // mesmo passo. Em dois inserts, uma falha no segundo deixaria jogador
  // órfão, fora da lista de quem chegou.
...
function dbFail(
  ...
  if (/champion_stays|champion_ids|rating|column/i.test(raw)) {
    return {
      ...
      available: 0,
      error:
        "Banco desatualizado — rode 0002_court_state.sql no SQL Editor do Supabase.",
    };
  }
  return {
    ok: false,
 ...
 */
export async function fetchHighlightDays(
  peladaId: string,
): Promise<HighlightDay[]> {
